<?php include('../templates/header_bulma.html');   ?>
  ...
  if($key == "Comestibles"){
  $query = "SELECT tiendas.id_tienda, tiendas.nombre, COUNT(compras.id_tienda) FROM tiendas, productos, compras WHERE productos.tipo NOT LIKE'%$producto%' 
  ...
  ORDER BY COUNT(compras.id_tienda) DESC LIMIT 5;";
  $result = $db -> prepare($query);
  $result -> execute();
  $tiendas = $result -> fetchAll(); 
  } 
  else {
  $query = "SELECT tiendas.id_tienda, tiendas.nombre, COUNT(compras.id_tienda) FROM tiendas, productos, compras WHERE productos.tipo LIKE'%$producto%' 
  AND productos.id_producto = compras.id_producto AND tiendas.id_tienda = compras.id_tienda GROUP BY tiendas.id_tienda, tiendas.nombre 
  ORDER BY COUNT(compras.id_tienda) DESC LIMIT 5;";
  $result = $db -> prepare($query);
  $result -> execute();
